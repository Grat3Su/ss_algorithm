...
//시뮬 + 순열 : 시간초과 주의
//src 기준으로 perm 제어
public class BOJ_17281_Baseball_perm {
	static int N, ans;
	static int[][] inning;
	static int[] src = {0,1,2,3,4,5,6,7,8};
	static int[] tgt =new int[9];// src와 tgt 숫자가 같을 때만 쓸 수 있다.
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = player; j < 9; j++) {
                ...
                if( num == 0 ) { // 아웃
                    ...
                    base = base << num;//주자이동
                    base = base | 1 << num - 1; //타자의 진루
                    
                    // 점수 계산
                    score += score(base);
                    
                    // 3루까지를 제외한 점수에 해당하는 주로 표시 초기화
                    base = base & ( ~(1 << 3) );//3번째 위치를 없애준다
                    base = base & ( ~(1 << 4) );
                    base = base & ( ~(1 << 5) );
                    base = base & ( ~(1 << 6) );
                }
